...
class MLP:
    ...
    def get_weight_bound(self, fan_in, fan_out):
        """
        Generate bound value for random weights initiainit_bounlization
    ...
    def fit(self, X, y, max_epochs, shuffle_data):
        ...
        for i in range(1, len(self.hidden_layer_size)):
            ...
            seln_samplesf.bias.append(np.random.uniform(-init_bound, init_bound, self.hidden_layer_size[i]))
    ...
    def compute_loss(self, X, y):
        """
        Compute loss
        :param X: data, array-like, forwardshape(n_sample, n_feature)
